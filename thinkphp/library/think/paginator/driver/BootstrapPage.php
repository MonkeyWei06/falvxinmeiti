    ...
    protected  function gopage()
    {
        ...
        if(!empty($_REQUEST['page'])){//分页进来带参数
            foreach($_REQUEST as $k=>$v){
                if($k == 'page'){
                    $params .="<input class='layui-input' type='number' min='1' name='page'  value='{$v}' style='display:inline-block;width:80px; line-height: 28px; padding-top: 5px; margin-top: 1px;' placeholder='请输入页数'>";
                }else{
            ...
            $params = "<input class='layui-input' type='number' min='1' name='page'  value='' style='display:inline-block;width:80px;' placeholder='页数'>";
        }

        return "<li><form action='' method='get' ><a class='layui-input-inline'>{$params} <input style='display:inline-block;line-height: 34px; background-color: #428bca; border: 0px;color: #fff;' class='layui-btn layui-btn-mini' type='submit' value='跳转'> </a></form></li>";
