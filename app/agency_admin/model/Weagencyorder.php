...
use think\Model;
use think\Db;
class Weagencyorder extends Model
{
    ...
    //会员订单及我的订单
    public function GetOrderList($UrlParam,$Where,$PageCount,$MemberTypeID,$AgencySiteInfo=array(),$type) //$type=1 是会员订单，2是我的订单
    {
        $Where=array_merge($UrlParam,$Where);
        $WhereArr=array();
        if(!empty($Where['title'])) $WhereArr['A.article_title|O.order_number']=['like','%'.trim($Where['title']).'%'];//文章标题
		if(!empty($Where['order'])) $WhereArr['O.order_number']=['like','%'.trim($Where['order']).'%'];
		if(!empty($Where['article_customer'])) $WhereArr['A.article_customer']=['like','%'.trim($Where['article_customer']).'%'];//文章标题
        if(!empty($Where['name'])) $WhereArr['M.MemberName']=['like','%'.trim($Where['name']).'%'];//用户名
        if(!empty($Where['startdate']) || !empty($Where['enddate'])){
            if(!empty($Where['startdate']) && !empty($Where['enddate'])){
                $Where['startdate']=strtotime($Where['startdate']);
                $Where['enddate']=strtotime($Where['enddate']);
                $begin_time=min($Where['startdate'],$Where['enddate']);
                $end_time=max($Where['startdate'],$Where['enddate']);$end_time+=86399;
                $WhereArr['O.order_time']=['between',[$begin_time,$end_time]];
            }else if(!empty($Where['startdate']) && empty($Where['enddate'])){
                $Where['startdate']=strtotime($Where['startdate'].' 00:00:00');
                $WhereArr['O.order_time']=['>=',$Where['startdate']];
            }else if(empty($Where['startdate']) && !empty($Where['enddate'])){
                $Where['startdate']=strtotime($Where['startdate'].' 00:00:00');
                $WhereArr['O.order_time']=['<=',$Where['startdate']];
            }
        ...
        if($type==1){//获得会员订单
           ...
           // $WhereArr['O.RegDomain']=$AgencySiteInfo['Domain'];//本域名的订单
		    $WhereArr['A.member_id']=$AgencySiteInfo['MemberID'];//本域名的订单
        ...
        $List=$this
            ->field('O.*,A.article_title,A.article_website,A.article_id,M.MemberName,MD.MediaTitle,A.article_customer')
            ->alias('O')//订单表
            ->where($WhereArr)
