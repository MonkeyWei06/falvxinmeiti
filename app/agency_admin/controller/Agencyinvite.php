...
use app\agency_admin\model\Invite;
use think\Request;

class Agencyinvite extends Agency
{
    ...
    public function getinvitelist(){
        ...
        $Where=array();
        $Where['InviteMemberID']=$this->AgencyMemberID;
		$Model=new Invite();
    ...
    public function add(){
        if($this->request->isPost()){
            ...
            $Post['InviteMemberID']=$this->AgencyMemberID;
            $Post['InviteState']=1;
    ...
    public function achievement()
    {
		...
		$this->assign('UrlParam',$UrlParam);
        $Where=array();
		$UrlParam["MemberID"]=$this->AgencyMemberID;
		$memberinvite=db("invite")->where('InviteMemberID','eq',$this->AgencyMemberID)->where('InviteCode','eq',$code)->find();
	...
	function ewmhb(){
		...
		$data = "http://".$_SERVER['SERVER_NAME']."/member.php/reg.html?invite=$code"; //生成二维码的内容
