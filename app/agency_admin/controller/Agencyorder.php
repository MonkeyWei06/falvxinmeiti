...
class Agencyorder extends Common
{
   ...
   //导出我的订单
    public function ExportMyOrder()
    {
        ...
        if(!empty($Info))
        {
            ...
            //if(empty($UrlParam['export'])) return json(['code'=>0,'msg'=>'存在数据']);
            //获取标题
    ...
    public function GetMediaOrderList($UrlParam,$Where,$PageCount,$MemberTypeID,$AgencySiteInfo=array())
    {
        ...
        if(!empty($List)){
            foreach ($List as $Key=>$Val){
                $List[$Key]['order_time']=date("Y-m-d H:i",$Val['order_time']);
                $List[$Key]['release_time']=$Val['release_time']>0?date("Y-m-d H:i",$Val['release_time']):'未发布';
                $List[$Key]['check']='<div class="bai"id="'.$Val['agencyorder_id'].'"  onclick="checka('.$Val['agencyorder_id'].')" ></div>';
    ...
    public function cancel(){
        ...
        $order_state = 4;
        $fabu_site = '';
        controller('common/Hezi')->cancel($ID);
        $fabu_content = '';
    ...
    */
	...
	public function editsave(){
			...
			if($data && $data['article_id']>0){
  			...
  			if($res){
				$this->success('修改成功','agencyorder/myorder');
			}else{
				$this->error('修改失败','agencyorder/myorder');
			}
		}
}