...
class Article extends Common
{
    ...
    public function deal_past(){
        ...
        foreach ($list as $key => $value) {
            $ArticleInfo=db('media')->where('MediaID',$value['media_id'])->find();
    ...
    public function index()
    {
        ...
        $ID = input('ID');
		$temp = input('temp');
        if(!empty($State) && $State=='Yes'){//再次发布
            $ArticleInfo=db('article')->where('article_id',$ID)->where('member_id',$this->MemberID)->find();
			$OrderInfo=db('agencyorder')->where('article_id',$ID)->where('media_type','1')->find();
    ...
    public function WordUpLoad($file)
    {
		...
		foreach($html_dom->find('img') as $e) {
			$new_src=$this->base64_image_content($e->src,ROOT_PATH.'public'.DIRECTORY_SEPARATOR.'uploads'. DIRECTORY_SEPARATOR.'word'. DIRECTORY_SEPARATOR);
			if(!$new_src) continue;
    ...
    public function sborder()
    {
        ...
        //if(!input('editorValue')) return '请上传文件';
        //if(!input('beizhu')) return '备注不能为空';
		...
		$list = Db::name('media')->where('MediaID','in',$idArr)->where('MediaState',1)->select();
		...
		{
			$AMoney+= controller('common/Common')->price($this->MemberInfo['MemberID'],$val['MediaID']);
			if($fuji)$AMoney2+= controller('common/Common')->price($fuji['MemberID'],$val['MediaID']);//父级的费用；二级；
			if($yeji)$AMoney3+= controller('common/Common')->price($yeji['MemberID'],$val['MediaID']);//爷级的费用；一级；
		}
		...
		$push_state = Db::name('member')
			...
			->value('MemberTypeID');  //MemberTypeID 会员类型，1 普通，2 二级，3 一级
		...
		db('article_temp')->where('TempArticleID',$TempArticleID)->where('member_id',$this->MemberID)->delete();//删除临时ID记录
		$statacheck=0;
		//Db::startTrans();
		//try {
			foreach ($list as $key=>$val)
			{
					...
					$Money = controller('common/Common')->price($this->MemberInfo['MemberID'],$val['MediaID']);
					$data = [];
    ...
    public function cancel(){
		...
		$mediatype = Db::name('media')->where('MediaID',$order['media_id'])->find();
