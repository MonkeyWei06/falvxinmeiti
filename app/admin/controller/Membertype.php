...
class Membertype extends Common
{
    private $MenuName = '会员分类';
    ...
    public function index()
    {
        ...
        $List = $this->ParentTypeList('membertype',$Keyword);
...
//            $this->success('添加成功');
            ...
            $this->assign('ParentTypeList',$this->ParentTypeList('membertype'));
            return $this->fetch();
        }
    }
    //保存数据
    private function AddSave($Data)
    {
        ...
        $Data['TypeTime']=time();
        //保存添加的数据
        Db::name('membertype')->data($Data)->insert();
        ...
        Db::name('membertype')->where($Where)->update($SaveData);
        //保存日志
    ...
    public function edit()
    {
        if($this->request->isPost())
        {
            ...
            $Info=Db::name('membertype')->where(['TypeID'=>$ID])->find();
            $this->assign('ParentTypeList',$this->ParentTypeList('membertype','','Edit',$Info['TypeID']));
...
//                $this->error('您的当前级别不可挪到小于您的当前级别之下');
        ...
        Db::name('membertype')->data($Data)->where('TypeID',$ID)->update();
        //更新当前层级的所有子集
        $List=Db::name('membertype')->field('TypeID,TypePath')->where('TypePath','like',$OldTypePath.'%')->select();
        if(!empty($List))
        {
            $UpdateData=array();
...
//        $this->success('排序更新成功');
    ...
    public function del()
    {
        if($this->request->isPost())
        {
            ...
            //dump($ID);
            if (!empty($ID)) {
                $Info = Db::name('membertype')->field('TypePath')->where('TypeID', $ID)->find();
                if (empty($Info)) {
