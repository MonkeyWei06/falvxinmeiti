...
use think\Db;
use app\admin\model\Notice as NewsModel;
...
//        $this->success('此'.$this->MenuName.'已添加成功');
    ...
    private function EditSave($Post)
    {
        ...
        $PicPath=$this->OneUpload('NewsPicPath');
        if(!empty($PicPath)){
            $this->DelOneOldPic($NewsID,'notice','NewsID','NewsPicPath');//先删除
            $Post['NewsPicPath']=$PicPath;
        }
        $Where=[];
        $Where['NewsID']=$NewsID;
...
//        $this->success('此'.$this->MenuName.'已修改成功');
    ...
    public function del()
    {
        if($this->request->isPost())
        {
            ...
            $ID = input('param.id');
            if (!empty($ID)) {
                $this->DelOneOldPic($ID, 'notice', 'NewsID', 'NewsPicPath');
                $Model = new NewsModel();
...
//            $this->success('此记录已删除成功');
    ...
    private function delmore()
    {
        ...
        if(!empty($DelID))
        {
            ...
            $this->DelOneOldPic($DelIDStr,'notice','NewsID','NewsPicPath');
            $Model=new NewsModel();
...
//        $this->success('排序更新成功');
    ...
    {
        $List=Db::name('newstype')->field('TypeID,TypePID,TypePath,TypeName')->where('TypePID',$PID)->order('TypeSort Asc,TypeID Asc')->select();
        return $List;
    }
    //Ajax分类返回调用
