...
use think\Db;
use \app\admin\model\Abouttype as AboutTypeModel;
class Abouttype extends Common
{
    private $MenuName = '关于万千分类';
    ...
    public function index()
    {
        ...
        $List = $this->ParentTypeList('abouttype',$Keyword);
    ...
    public function add()
    {
        if($this->request->isPost())
        {
            ...
            //获取父级分类列表
            $this->assign('ParentTypeList',$this->ParentTypeList('abouttype'));
            return $this->fetch();
        }
    }
    //保存数据
    private function AddSave($Data)
    {
        ...
        $Data['TypeTime']=time();
        //保存添加的数据
        Db::name('abouttype')->data($Data)->insert();
        ...
        Db::name('abouttype')->where($Where)->update($SaveData);
        //保存日志
    ...
    public function edit()
    {
        if($this->request->isPost())
        {
            ...
            $Info=Db::name('abouttype')->where(['TypeID'=>$ID])->find();
            $this->assign('ParentTypeList',$this->ParentTypeList('abouttype','','Edit',$Info['TypeID']));
    ...
    private function EditSave($ID,$Data)
    {
        ...
        Db::name('abouttype')->data($Data)->where('TypeID',$ID)->update();
        //更新当前层级的所有子集
        $List=Db::name('abouttype')->field('TypeID,TypePath')->where('TypePath','like',$OldTypePath.'%')->select();
        if(!empty($List))
        {
            $UpdateData=array();
    ...
    public function del()
    {
        if($this->request->isPost())
        {
            ...
            //dump($ID);
            if (!empty($ID)) {
                $Info = Db::name('abouttype')->field('TypePath')->where('TypeID', $ID)->find();
                if (empty($Info)) {
