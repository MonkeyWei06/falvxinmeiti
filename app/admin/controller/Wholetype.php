...
class Wholetype extends Common
{
    private $MenuName = '商品分类';
    ...
    public function index()
    {
        ...
        $List = $this->ParentTypeList('wholetype',$Keyword);
...
//            $this->success('添加成功');
            ...
            //获取父级分类列表
            $this->assign('ParentTypeList',$this->ParentTypeList('wholetype'));
            return $this->fetch();
        }
    }
    //保存数据
    private function AddSave($Data)
    {
        ...
        $Data['TypeTime']=time();
//        //多文件上传
//        $PicPath=$this->MoreUpload('TypePicPath');
//        if(!empty($PicPath)) {
//            foreach ($PicPath as $key=>$val) {
//                $Data['TypePicPath'.($key+1)]=$val;
//            }
//        }
        //保存添加的数据
        Db::name('wholetype')->data($Data)->insert();
        ...
        Db::name('wholetype')->where($Where)->update($SaveData);
        //保存日志
    ...
    public function edit()
    {
        if($this->request->isPost())
        {
            ...
            $Info=Db::name('wholetype')->where(['TypeID'=>$ID])->find();
            $this->assign('ParentTypeList',$this->ParentTypeList('wholetype','','Edit',$Info['TypeID']));
...
//            foreach ($PicPath as $key=>$val) {
//                $Data['TypePicPath'.($key+1)]=$val;
//                $DelFieldPicPath[]='TypePicPath'.($key+1);
//            }
//            //删除多条记录的多个字段的图片
//            $this->DelMoreOldPic($ID,'wholetype','TypeID',$DelFieldPicPath);
//        }
        Db::name('wholetype')->data($Data)->where('TypeID',$ID)->update();
        //更新当前层级的所有子集
        $List=Db::name('wholetype')->field('TypeID,TypePath')->where('TypePath','like',$OldTypePath.'%')->select();
        if(!empty($List))
        {
            $UpdateData=array();
...
//        $this->success('排序更新成功');
    ...
    public function del()
    {
        if($this->request->isPost())
        {
            ...
            //dump($ID);
            if (!empty($ID)) {
                $Info = Db::name('wholetype')->field('TypePath')->where('TypeID', $ID)->find();
                if (empty($Info)) {
//                    $this->error('此记录已删除，请不要重复操作');
                ...
                //删除多条记录的多个字段的图片
                $this->DelMoreOldPic($DelIDTempStr, 'wholetype', 'TypeID', ['TypePicPath1', 'TypePicPath2']);
                Db::name('wholetype')
                    ->where(['TypeID' => $ID])
                    ->whereOr('TypePath', 'like', $TypePath)
                    ->delete();
