...
class Media extends Model
{
    ...
    public function GetList($UrlParam,$PageCount)
    {
        ...
        if(!empty($UrlParam['keyword'])){
            $Where['t1.MediaTitle']=array('like','%'.trim($UrlParam['keyword']).'%');
        }
		...
		if(!empty($UrlParam['keyword2'])){
            $Where['M.MemberName']=array('like','%'.trim($UrlParam['keyword2']).'%');
			$List=$this->alias('t1')
            ->field('t1.*,M.MemberName')
			...
			->join('member M','M.MemberID=t1.MediaMemberID','left')
            ->where($Where)->order('t1.UserType asc,t1.MediaID Desc')
            ->paginate($PageCount,false,['query'  => $UrlParam]);
			foreach ($List as $key=>$val) {
				if($val['MediaMemberID'] >0){
						$List[$key]['membername']=\think\Db::name('member')->where('MemberID',$val['MediaMemberID'])->value('MemberName');
					}
			}
        }else{
			$List=$this->alias('t1')
